...
class PlanningMapRenderer:
    ...
    def create_traffic_light_icon(self, sx: float, sy: float):
        shapes = []
        box_w, box_h = 16, 42 
        light_radius = 5 
        spacing = 11 
    ...
    def draw_topology(self, topology: dict, canvas_static: cv.Canvas, canvas_dynamic: cv.Canvas, drawn_nodes_cache: list):
        if not topology: return
        ...
        for node in nodes_iter:
            try:
                ...
                if is_traffic_light:
                    ...
                    icon_shapes = self.create_traffic_light_icon(cx, cy)
