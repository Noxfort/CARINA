...
class XaiClient:
    ...
    def _get_agent_list_sync(self) -> List[str]:
        """
        Reads status.json from the active scenario and returns the list of agent IDs.
        Private method intended to be run in a thread.
        """
        active_scenario_path = self._get_active_scenario_path()
        if not active_scenario_path:
            return []
        try:
            status_file_path = os.path.join(active_scenario_path, "status.json")
