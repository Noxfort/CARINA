...
class AnalyzerDataProcessor:
    """Processes raw simulation and database historical data for the AnalyzerEngine."""
    ...
    def process_historical_data(self, db_manager, net_file_path: str, limit_seconds: int = None) -> tuple[dict, list]:
        ...
        logging.info("[ANALYZER_DATA_PROCESSOR] Processing historical data from database in batches...")
        ...
        if net_file_path:
            try:
                ...
                logging.warning(f"[ANALYZER_DATA_PROCESSOR] Failed to parse net file topology ({e}). Will fallback to synthetic topology.")
        ...
        try:
            if hasattr(db_manager, "query_aggregated_fluid_dynamics"):
                ...
                if aggregated_rows:
                    logging.info(f"[ANALYZER_DATA_PROCESSOR] Pushdown Query executed successfully: {len(aggregated_rows)} aggregated edge summaries loaded in C/SQL speed.")
            ...
            logging.warning(f"[ANALYZER_DATA_PROCESSOR] Pushdown query failed ({e}), falling back to batch iteration.")
            edge_summaries = {}
            total_samples = 0
            ...
            try:
                ...
                for batch in batch_generator:
                    ...
                    for sample in batch:
                        ...
                        # MEMORY FIX: Bin queue lengths into groups of 5 to limit dict key count
                        q_len = int(sample.get('queue_length', 0))
                        q_bin = (q_len // 5) * 5
                        summary['queue_freq'][q_bin] += 1
            except Exception as e:
                logging.error(f"[ANALYZER_DATA_PROCESSOR] Error processing historical data batches: {e}", exc_info=True)

        logging.info(f"[ANALYZER_DATA_PROCESSOR] Processed {total_samples} traffic samples from database in batches.")

        if total_samples == 0:
            return {}, true_traffic_light_ids

        # 3. Create representative/synthetic samples for each edge to keep downstream strategies unchanged
        samples_by_edge = {}
        edge_volumes_cache = {}
        ...
        if not junction_incoming_edges and samples_by_edge:
            logging.info("[ANALYZER_DATA_PROCESSOR] Building synthetic fallback topology for database edges.")
            synth_dict = {}
        ...
        for j_id, incoming_edges in junction_incoming_edges.items():
            ...
            for edge_id in incoming_edges:
                edge_volumes[edge_id] = edge_volumes_cache.get(edge_id, 0.0)
        ...
        logging.info(f"[ANALYZER_DATA_PROCESSOR] Processed {len(processed_data)} junctions from historical data using batching.")
